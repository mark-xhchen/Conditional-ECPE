...
tf.app.flags.DEFINE_integer('num_heads', 2, 'number of heads of self attention')
...
def build_model(word_embedding, x, sen_len, doc_len, keep_prob1, keep_prob2, y, emo, cau, con, RNN=biLSTM):
    ...
    # [-1, max_cau_num, max_doc_len] * [-1, max_doc_len, 2*hidden] = [-1, max_cau_num, 2*hidden]
    s_cau = tf.matmul(cau, s)
    s_wc = tf.concat([s_emo, s_cau], 1)
    s_sen = trans_func(s_wc, s, 2*FLAGS.n_hidden, 'partial_attention')

    loss_wc = tf.constant(0, tf.float32)
    reg_wc = tf.constant(0, tf.float32)

    with tf.name_scope('context_prediction'):
        pred_wc, reg_wc = softmax_part(s_sen, 4*2*FLAGS.n_hidden, keep_prob2, FLAGS.n_class, 'softmax_w_wc', 'softmax_b_wc')

        pred_wc += 1e-5
        pred_wc /= tf.reshape(tf.reduce_sum(pred_wc, 1), [-1, 1])

        loss_wc = -tf.reduce_sum(y * tf.log(pred_wc)) / tf.cast(tf.shape(x)[0], tf.float32)

    loss = loss_wc
    reg = reg_wc

    return loss, pred_wc, reg
...
def trans_func(s_q, s, n_feature, scope_var):
    senEncode_assist = trans.multihead_attention(queries=s_q,
                                            keys=s,
                                            values=s,
                                            ...
                                            is_training=True,
                                            scope=scope_var)
    #senEncode_assist = trans.feedforward_1(senEncode_assist, out_units, out_units)
...
def run():
    ...
    # Model Code Block
    word_idx_rev, word_id_mapping, word_embedding, _ = load_w2v(FLAGS.embedding_dim, FLAGS.embedding_dim_pos, './nega_data/clause_keywords.csv', FLAGS.w2v_file)
