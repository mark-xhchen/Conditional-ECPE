...
def build_model(word_embedding, x, sen_len, doc_len, keep_prob1, keep_prob2, y, emo, cau, con, RNN=biLSTM):
    ...
    s_sen = RNN(s, doc_len, n_hidden=FLAGS.n_hidden, scope=FLAGS.scope + 'sentence_lstm_layer')
    ...
    with tf.name_scope('context_prediction'):
        ...
        s_emo = tf.matmul(emo, s_sen)
        # [-1, max_cau_num, max_doc_len] * [-1, max_doc_len, 2*hidden] = [-1, max_cau_num, 2*hidden]
        s_cau = tf.matmul(cau, s_sen)
        s_wc = tf.reshape(tf.concat([s_emo, s_cau], 1), [-1, 4*2*FLAGS.n_hidden])
        #pred_wc, reg_wc = softmax_part(s_sen, FLAGS.max_doc_len*2*FLAGS.n_hidden, keep_prob2, FLAGS.n_class, 'softmax_w_wc', 'softmax_b_wc')
