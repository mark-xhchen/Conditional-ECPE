...
def build_model(word_embedding, x, sen_len, doc_len, keep_prob1, keep_prob2, y, emo, cau, con, RNN=biLSTM):
    ...
    with tf.name_scope('context_prediction'):
        ...
        pred_woc, reg_woc = softmax_part(s_wc, 4*2*FLAGS.n_hidden, keep_prob2, FLAGS.n_class, 'softmax_w_woc', 'softmax_b_woc')
...
def trans_func(s_q, s, n_feature, scope_var):
    senEncode_assist = trans.multihead_attention(queries=s_q,
                                            ...
                                            is_training=True,
                                            scope=scope_var)
    #senEncode_assist = trans.feedforward_1(senEncode_assist, n_feature, n_feature)
